...
import { IHighRiskSiteSetupState } from './IHighRiskSiteSetupState';
...
export default class HighRiskSiteSetup extends React.Component<IHighRiskSiteSetupProps,IHighRiskSiteSetupState> {
  constructor(props: IHighRiskSiteSetupProps) {
    super(props);
  ...
  private siteChanged(option: IDropdownOption, idx?: number) {
    ...
    getListFromWeb(option.key as string, this.props.highRiskListName).then(list => {
      this.addMessage(`Required List ${this.props.highRiskListName} was found on that site`);
      ...
      if (fieldsfound) {
        this.addMessage(`Required fields found on List ${this.props.highRiskListName} `);
      ...
      this.addMessage(`<h1>List ${this.props.highRiskListName} was not found on that site</h1>`);
      this.setState((current) => ({
        ...current,
  ...
  private async ConvertUsersInHighRisk() {
    ...
    if (!ensureFieldsAreInList(this.state.highRiskList, ["PrimaryApprover"], this.addMessage)) {
      this.addMessage(`Creating PrimaryApprover Column in '${this.state.highRiskList["Title"]}'`);
      await addPeopleFieldToList(this.state.webUrl, this.props.highRiskListName, "PrimaryApprover", "PrimaryApprover").then(d => {
        this.addMessage(`Created PrimaryApprover Column  in '${this.state.highRiskList["Title"]}'`);

      }).catch(err => {
        this.addMessage(`<h1>There was an error adding the PrimaryApprover column to the ${this.state.highRiskList["Title"]} list</h1>`);
      });

    } else {
      this.addMessage(`PrimaryApprover Column already exists in '${this.state.highRiskList["Title"]}'`);
    }

    this.addMessage(`Updating PrimaryApprover column from ApproverEmail  in '${this.state.highRiskList["Title"]}'`);
    await convertEmailColumnsToUser(this.state.webUrl, this.props.highRiskListName, [["ApproverEmail", "PrimaryApproverId"]], this.addMessage);
    this.addMessage(`Updated PrimaryApprover column from ApproverEmail  in '${this.state.highRiskList["Title"]}'`);
    ...
    return (
      <div className={styles.highRiskSiteSetup}>
        ...
        <table style={{ border: 1 }}>
          ...
          <tr>
            <td>
              {this.props.highRiskListName}
            </td>

            <td>
              {this.state.highRiskListExists ? "Yes" : "No"}
            </td>

            <td>
              {this.state.highRiskFieldsFound ? "Yes" : "No"}
            </td>
            ...
            <td>
              <IconButton
                iconProps={{ iconName: "DocumentApproval" }}
                onClick={this.ConvertUsersInHighRisk.bind(this)} >
                Convert users
              </IconButton>
            </td>
          </tr>
