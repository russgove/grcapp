...
import { IHighRiskProps } from './IHighRiskProps';
import { IHighRiskState } from './IHighRiskState';
...
import {HighRiskItem,RoleToTransaction} from "../dataModel";
import { find, map, clone, filter } from "lodash";

export default class HighRisk extends React.Component<IHighRiskProps, IHighRiskState> {
  private selection: Selection = new Selection();
  public constructor(props: IHighRiskProps) {
    super();
  ...
  public componentDidMount() {
    this.props.fetchHighRisks().then(highrisks => {
      debugger;
  ...
  public RenderApproval(item?: HighRiskItem, index?: number, column?: IColumn): JSX.Element {
      ...
      return (
        <Dropdown options={options}
          ...
          onChanged={(option: IDropdownOption, idx?: number) => {
            let tempRoleToTCodeReview = this.state.highRisks;
  ...
  public RenderComments(item?: HighRiskItem, index?: number, column?: IColumn): JSX.Element {
    if (this.props.primaryApprover[0].Completed === "Yes") {
      return (
        <div>
          {item.Comments}
  ...
  public save(): Promise<any> {
    return this.props.save(this.state.highRisks).then(() => {
      debugger;
      var tempArray = map(this.state.highRisks, (rr) => {
  ...
  // }
  public showPopup(item: HighRiskItem) {

    this.props.getRoleToTransaction(item.Role_x0020_Name)
  ...
  public fetchHighRisks(): Promise<any> {
    debugger;
    return this.props.fetchHighRisks().then((highrisks) => {
      debugger;
      this.setState((current) => ({ ...current, highRisk: highrisks }));
    }).catch((err) => {
      debugger;
  ...
  public render(): React.ReactElement<IHighRiskProps> {
    ...
    let itemsNonFocusable: IContextualMenuItem[] = [
   ...
   
      {
        ...
        disabled: !(filter(this.state.highRisks, (rr) => { return rr
      ...
      { // if the item has been comleted OR there are items with noo approvasl, diable
        ...
        disabled: this.props.primaryApprover[0].Completed === "Yes" ||
        (filter(this.state.highRisks, (rr) => { return rr.Approval === null; }).length > 0)
      }
    ...
    let farItemsNonFocusable: IContextualMenuItem[] = [
      {
        ...
        disabled: !(filter(this.state.highRisks, (rr) => { return rr.hasBeenUpdated; }).length > 0)
